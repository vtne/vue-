          ...
          >
            ...
            <el-table-column label="诉求类型">
              <template slot-scope="scope">
                <span>{{ scope.row.appealType | typeFormat }}</span>
              </template>
            </el-table-column>
...
export default {
  ...
  methods: {
    // 获取诉讼类别
    getEventLEvel () {
      this.$http.get('/report/queryType').then(res => {
        this.eventLevelOptions = res.data.data
      })
    },
    // 获取任务列表
    getList () {
...
.eventManage .seeBox .title span {
  margin-left: 50px;
}
.eventManage .seeBox .main div {
  font-size: 20px;
  margin-top: 40px;
  margin-left: 30px;
}
.eventManage .seeBox .main div.photo {
  width: 800px;
  height: 200px;
  border: 1px solid #CECECE;
}
.eventManage .seeBox .main div.photo ul li {
  float: left;
  margin-top: 40px;
  margin-left: 20px;
}
.eventManage .seeBox .main button {
  margin-left: 480px;
  margin-top: 20px;
}
/* 列表 */
